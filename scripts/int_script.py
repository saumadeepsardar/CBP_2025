...
def get_int_traces(trace_dir):
    int_dir = os.path.join(trace_dir, "int")
    if not os.path.exists(int_dir):
        raise FileNotFoundError(f"Directory {int_dir} does not exist")
    traces = [os.path.join(int_dir, f) for f in os.listdir(int_dir) if f.endswith('_trace.gz')]
    return traces

my_traces = get_int_traces(trace_dir)

def process_run_op(pass_status, my_trace_path, my_run_name, op_file):
    run_name_split = re.split(r"/", my_run_name)
    wl_name = run_name_split[0]  # e.g., "int"
    run_name = run_name_split[1]  # e.g., "int_0_trace"
...
def execute_trace(my_trace_path):
    ...
    my_wl = run_split[-2]  # e.g., "int"
    run_name = run_split[-1].split(".")[0]  # e.g., "int_0_trace"
...
if __name__ == '__main__':
    print(f'Running {len(my_traces)} traces from int folder: {my_traces}')
    ...
    df.to_csv(f'{results_dir}/uncond_branch_int_results_{timestamp}.csv', index=False)

    print('\n--- Aggregate Metrics for Unconditional Branches (int) ---')
