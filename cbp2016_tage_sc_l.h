...
public:
  ...
  CBP2016_TAGE_SC_L(void) {
    ...
    ltable = nullptr;
    for (int i = 1; i <= NHIST; i++) gtable[i] = nullptr;
    ...
    void init_histories(cbp_hist_t& current_hist) {
        ...
        NOSKIP[4] = 0; NOSKIP[NHIST - 2] = 0; NOSKIP[8] = 0; NOSKIP[NHIST - 6] = 0;
        ...
        for (int i = 0; i < HISTBUFFERLENGTH; i++) current_hist.ghist[i] = 0;
        current_hist.ptghist = 0;

        // Initialize alternate prediction table
        for (int i = 0; i < SIZEUSEALT; i++) use_alt_on_na[i] = 0;
 ...
 /** Compute state with enriched features */
  ...
  if (pred_taken == resolveDir) {
      ...
      reward = HighConf ? -1.0f : -0.5f; // Penalty based on confidence
        ...
        if (pred_taken == resolveDir && (MYRANDOM() & 31) != 0) ALLOC = false;
        ...
        if (LVALID) {
            if (resolveDir != predloop) {
                if (ltable[LI].CurrentIter + 1 == ltable[LI].NbIter) {
                    if (ltable[LI].confid < 15) ltable[LI].confid++;
                } else {
                    ltable[LI].NbIter = 0;
                    ltable[LI].age = 0;
                    ltable[LI].confid = 0;
                ...
                if (ltable[LI].CurrentIter + 1 != ltable[LI].NbIter) {
                    if (ltable[LI].age < 15) ltable[LI].age++;
                }
            }
